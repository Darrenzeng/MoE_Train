...
# Create a Qwen2 MoE from a set of equally-sized Qwen2 models.
...
class Qwen2MOEConfig(BaseModel):
    base_model: str
    experts: List[Expert]
...
def is_bad_config(config: Qwen2MOEConfig, allow_all_same: bool = False) -> bool:
...
def build(
    ...
    for weight_info in arch_info.pre_weights(base_cfg) + arch_info.post_weights(
        base_cfg
    ):
    ...
    for layer_idx in range(base_cfg.num_hidden_layers):
        for weight_info in arch_info.layer_weights(index=layer_idx, config=base_cfg):
            tensor_name = weight_info.name

            if ".mlp." in tensor_name:
                for moe_index, expert in enumerate(config.experts):
                    expert_name = tensor_name.replace(
                        ".mlp.gate_proj", f".moe.mlp.{moe_index}.gate_proj"
                    )
                    expert_name = expert_name.replace(
                        ".mlp.down_proj", f".moe.mlp.{moe_index}.down_proj"
                    )
                    expert_name = expert_name.replace(
                        ".mlp.up_proj", f".moe.mlp.{moe_index}.up_proj"
                    )
    ...
    for layer_idx in range(base_cfg.num_hidden_layers):
        writer.save_tensor(
            f"model.layers.{layer_idx}.moe.gate.weight",
...
@click.command("mergekit-moe-qwen2")
...
def main(
    ...
    config = Qwen2MOEConfig.model_validate(yaml.safe_load(config_source))
    build(
        config,
        out_path=out_path,
